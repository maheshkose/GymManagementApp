...
import "./AttendenceEmployees.css";


const AttendenceEmployees = () => {
  const {
    getAllAttendenceTrendOfEmployee,getAttendenceOfTodayEmployee,getCurrentlyInGymEmployee,topattendingEmployee
  } = AppContextHook();
  ...
  const [topAttendingEmployeeList, settopAttendingEmployeeList] = useState([]);
  const [weeklyAttendenceTrend, setweeklyAttendenceTrend] = useState([]);

  const getAllAttendenceTrendOfEmployeeHandler = async () => {
    const res = await getAllAttendenceTrendOfEmployee();
    if (res?.data?.success) {
  ...
  const getAttendenceOfTodayEmployeeHandler = async () => {
    const res = await getAttendenceOfTodayEmployee();
    if (res?.data?.success) {
      toast.success(res.data.message);
  ...
  const getCurrentlyInGymEmployeeHandler = async () => {
    const res = await getCurrentlyInGymEmployee();
    if (res?.data?.success) {
      toast.success(res.data.message);
  ...
  //   }
  // };
  const topattendingEmployeeHandler = async () => {
    const res = await topattendingEmployee();
    if (res?.data?.success) {
      toast.success(res.data.message);
      settopAttendingEmployeeList(res.data.attendences);
    } else {
      toast.error(res.response?.data?.message);
    }
  };
  ...
  useEffect(() => {
    getAttendenceOfTodayEmployeeHandler();getCurrentlyInGymEmployeeHandler();
    getAllAttendenceTrendOfEmployeeHandler();
    topattendingEmployeeHandler();
  ...
  const attendanceSearchHandler = (e) => {
    ...
    const filteredCheckins = todayAllCheckinsArray.filter((checkin) =>
      checkin.employee.name.toLowerCase().includes(searchTerm)
    );
  ...
  return (
    <div className="attendance-page">
      <div className="att-track">
        <h2>Employees Attendance Tracking</h2>
        <p>This is where Employees attendance details will be displayed and managed.</p>
      </div>
      ...
      <div className="today-chekins-list-container">
       ...
       <div className="topperformers">
          ...
          <p>most consistent employees this week</p>
          <div className="top-prf-con">
            {topAttendingEmployeeList && topAttendingEmployeeList.length > 0 ? (
              topAttendingEmployeeList?.map((memberAtt, i) => (
                <div key={i} className="att-card">
                  <p className={`rank ${i+1 === 1? "gold": i+1 > 1 && i+1 <4?" silver":"bronze"}`}>#{i + 1}</p>
                  <div className="name-days">
                    <p className="name">{memberAtt.employee.name}</p>
